...
export async function logDataToDb(userId,email) {
  ...
  console.log(`error while logging the data to DB -> ${error}`)
}
}
...
export async function getEmployeeData(empId) {
  ...
  try {
    ...
    const employeeData = results.length > 0 ? results[0] : null;
    const message = results.length > 0 ? 'Employee data:' : `Employee not found with ID: ${empId}`;
